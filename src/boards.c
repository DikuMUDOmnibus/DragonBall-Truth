...
struct board_info_type board_info[NUM_OF_BOARDS] = {
  {3099, 1, 1, 1, LIB_ETC"board.mort", 0},
  {3098, LVL_IMMORT, LVL_IMMORT, LVL_GRGOD, LIB_ETC"board.immort", 0},
  {3097, 1, 1, 1, LIB_ETC"board.freeze", 0},
  {3096, 1, 1, 1, LIB_ETC"board.social", 0},
  {5892, 1, 1, 1, LIB_ETC"board.clan3", 0},
  {253, LVL_IMMORT, LVL_IMMORT, LVL_GRGOD, LIB_ETC"board.builder", 0},
  {254, LVL_IMMORT, LVL_IMMORT, LVL_IMMORT, LIB_ETC"board.enforcer", 0},
  {5898, 1, 1, 1, LIB_ETC"board.clan1", 0},
  {5899, 1, 1, 1, LIB_ETC"board.clan2", 0},
  {5799, 1, 1, 1, LIB_ETC"board.clan4", 0},
  {2010, 1, 1, 1, LIB_ETC"board.clan5", 0},
  {16199, 1, 1, 1, LIB_ETC"board.chat", 0},
};
...
int Board_write_message(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  ...
  *(tmstr + strlen(tmstr) - 1) = '\0';
  ...
  if (GET_SEX(ch) == SEX_FEMALE) {
   sprintf(buf, "&00&06%6.10s &16(&13%-12s&16) &15::&00 %s", tmstr, buf2, arg);
   }
  else if (GET_SEX(ch) == SEX_NEUTRAL) {
   sprintf(buf, "&00&06%6.10s &16(&10%-12s&16) &15::&00 %s", tmstr, buf2, arg);
   }
  else
  sprintf(buf, "&00&06%6.10s &16(&14%-12s&16) &15::&00 %s", tmstr, buf2, arg);
...
int Board_show_board(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  ...
  strcpy(buf,
	 "&15This is a bulletin board&15.  &00&06Usage&16: &10READ&16/&09REMOVE &16<&14messg #&16>&15, &00&06WRITE &16<&14header&16>&15.&00\r\n"
	 "&15Type /h while editing a board message for help on writing commands.&00\r\n");
  if (!num_of_msgs[board_type])
    strcat(buf, "&15The board is empty.&00\r\n");
  else {
    sprintf(buf + strlen(buf), "&15There are &11%d &15message&16(&15s&16)&15 on the board&15.&00\r\n\r\n",
	    num_of_msgs[board_type]);
#if NEWEST_AT_TOP
...
#if NEWEST_AT_TOP
	sprintf(buf + strlen(buf), "&11%-2d &15: %s\r\n",
...
int Board_display_msg(int board_type, struct char_data * ch, char *arg, struct obj_data *board)
{
  ...
  sprintf(buffer, "&00&02Message &11%d &15: %s\r\n\r\n%s\r\n", msg,
