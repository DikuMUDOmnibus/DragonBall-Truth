...
void affect_total(struct char_data * ch)
{
  ...
  i = (IS_NPC(ch) ? 20 : 100);
  if (GET_CHA(ch) < 125 && GET_CHA(ch) > 0) {
  GET_CHA(ch) = GET_CHA(ch);
  }
  if (GET_CHA(ch) < 0) {
  GET_CHA(ch) = 125;
  }
  ...
  if (GET_CON(ch) < 125 && GET_CON(ch) > 0) {
  GET_CON(ch) = GET_CON(ch);
  }
  if (GET_CON(ch) < 0) {
  GET_CON(ch) = 125;
  }
  ...
  if (GET_INT(ch) < 125 && GET_INT(ch) > 0) {
  GET_INT(ch) = GET_INT(ch);
  }
  if (GET_INT(ch) < 0) {
  GET_INT(ch) = 125;
  }
  ...
  if (GET_WIS(ch) < 125 && GET_WIS(ch) > 0) {
  GET_WIS(ch) = GET_WIS(ch);
  }
  if (GET_WIS(ch) < 0) {
  GET_WIS(ch) = 125;
  }
  ...
  GET_RAGE(ch) = MAX(0, MIN(GET_RAGE(ch), i));
  GET_UBS(ch) = MAX(0, MIN(GET_UBS(ch), i));
  GET_LBS(ch) = MAX(0, MIN(GET_LBS(ch), i));
...
void char_to_room(struct char_data * ch, room_rnum room)
{
  if (ch == NULL || room < 0 || room > top_of_world)
    log("SYSERR: Illegal value(s) passed to char_to_room. (Room: %d/\r\n%d \r\nCh: %p",
...
void equip_char(struct char_data * ch, struct obj_data * obj, int pos)
{
  ...
  if (IS_OBJ_STAT(obj, ITEM_DIVINE) && GET_MAX_HIT(ch) <= 2999999) {
   act("You are zapped by $p and instantly let go of it.", FALSE, ch, obj, 0, TO_CHAR);
   act("$n is zapped by $p and instantly lets go of it.", FALSE, ch, obj, 0, TO_ROOM);
   obj_to_char(obj, ch);     /* changed to drop in inventory instead of
                                 * ground */
  ...
  if (IS_OBJ_STAT(obj, ITEM_UBERDIVINE) && GET_MAX_HIT(ch) <= 49999999) {
   act("You are zapped by $p and instantly let go of it. You get the feeling that you need to be over 50 million pl to equip $p", FALSE, ch, obj, 0, TO_CHAR);
   act("$n is zapped by $p and instantly lets go of it.", FALSE, ch, obj, 0, TO_ROOM);
   obj_to_char(obj, ch);     /* changed to drop in inventory instead of
                                 * ground */
   return;
...
struct obj_data *unequip_char_for_rent(struct char_data * ch, int pos)
{
  int j;
  struct obj_data *obj;
...
void obj_to_obj(struct obj_data * obj, struct obj_data * obj_to)
{
  ...
  for (tmp_obj = obj->in_obj; tmp_obj->in_obj; tmp_obj = tmp_obj->in_obj)
    GET_OBJ_WEIGHT(tmp_obj) == GET_OBJ_WEIGHT(tmp_obj);

  /* top level object.  Subtract WEIGHT from inventory if necessary. */
  GET_OBJ_WEIGHT(tmp_obj) == GET_OBJ_WEIGHT(tmp_obj);
  if (tmp_obj->carried_by)
    IS_CARRYING_W(tmp_obj->carried_by) == GET_OBJ_WEIGHT(tmp_obj);
}
...
void obj_from_obj(struct obj_data * obj)
{
  ...
  for (temp = obj->in_obj; temp->in_obj; temp = temp->in_obj)
    GET_OBJ_WEIGHT(temp) == GET_OBJ_WEIGHT(temp);

  /* Subtract WEIGHT from char that carries the object */
  GET_OBJ_WEIGHT(temp) == GET_OBJ_WEIGHT(temp);
  if (temp->carried_by)
    IS_CARRYING_W(temp->carried_by) == GET_OBJ_WEIGHT(temp);
...
char *money_desc(int amount)
{
  ...
  if (amount == 1)
    strcpy(buf, "&00&07a &11gold&00&07 coin");
  else if (amount <= 10)
    strcpy(buf, "&00&07a tiny pile of &11gold&00&07 coins");
  else if (amount <= 20)
    strcpy(buf, "&00&07a handful of &11gold&00&07 coins");
  else if (amount <= 75)
    strcpy(buf, "&00&07a little pile of &11gold&00&07 coins");
  else if (amount <= 200)
    strcpy(buf, "&00&07a small pile of &11gold&00&07 coins");
  else if (amount <= 1000)
    strcpy(buf, "&00&07a pile of &11gold&00&07 coins");
  else if (amount <= 5000)
    strcpy(buf, "&00&07a big pile of &11gold&00&07 coins");
  else if (amount <= 10000)
    strcpy(buf, "&00&07a large heap of &11gold&00&07 coins");
  else if (amount <= 20000)
    strcpy(buf, "&00&07a huge mound of &11gold&00&07 coins");
  else if (amount <= 75000)
    strcpy(buf, "&00&07an enormous mound of &11gold&00&07 coins");
  else if (amount <= 150000)
    strcpy(buf, "&00&07a small mountain of &11gold&00&07 coins");
  else if (amount <= 250000)
    strcpy(buf, "&00&07a mountain of &11gold&00&07 coins");
  else if (amount <= 500000)
    strcpy(buf, "&00&07a huge mountain of &11gold&00&07 coins");
  else if (amount <= 1000000)
    strcpy(buf, "&00&07an enormous mountain of &11gold&00&07 coins");
  else
    strcpy(buf, "&00&07an absolutely colossal mountain of &11gold&00&07 coins");
