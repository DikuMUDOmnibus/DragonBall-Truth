...
sh_int r_trans_space;
sh_int r_trans_earth;
sh_int r_trans_vegeta;
sh_int r_trans_frigid;
sh_int r_trans_namek;
sh_int r_trans_orainas;
sh_int r_trans_konack;
sh_int r_hell_room;
sh_int r_demon_start_room;
sh_int r_saiyan_start_room;
sh_int r_konack_start_room;
sh_int r_angel_start_room;
sh_int r_icer_start_room;
sh_int r_namek_start_room;
sh_int r_truffle_start_room;
sh_int r_majin_start_room;
sh_int r_transport_earth;
sh_int r_pk1_room;
sh_int r_pk2_room;
sh_int r_pk3_room;
sh_int r_pk4_room;
sh_int r_death_start_room;	/* rnum of mortal start room	 */
sh_int r_immort_start_room;	/* rnum of immort start room	 */
sh_int r_frozen_start_room;	/* rnum of frozen start room	 */
sh_int r_ctf_start_room;	/* rnum of frozen start room	 */
...
extern sh_int trans_space;
extern sh_int trans_earth;
extern sh_int trans_vegeta;
extern sh_int trans_orainas;
extern sh_int trans_konack;
extern sh_int trans_namek;
extern sh_int trans_frigid;
extern sh_int hell_room;
extern sh_int demon_start_room;
extern sh_int majin_start_room;
extern sh_int saiyan_start_room;
extern sh_int angel_start_room;
extern sh_int konack_start_room;
extern sh_int transport_earth;
extern sh_int pk1_room;
extern sh_int pk2_room;
extern sh_int pk3_room;
extern sh_int pk4_room;
extern sh_int icer_start_room;
extern sh_int truffle_start_room;
extern sh_int namek_start_room;
extern sh_int death_start_room;
extern sh_int immort_start_room;
extern sh_int frozen_start_room;
extern sh_int ctf_start_room;
...
ACMD(do_reboot)
{
  ...
  if (!str_cmp(arg, "all") || *arg == '*') {
    ...
    file_to_string_alloc(STARTUP2_FILE, &startup2);
    file_to_string_alloc(SYSLOG_FILE, &syslog);
  ...
  for (; !feof(player_fl);) {
    ...
    if (!feof(player_fl)) {	/* new record */
      ...
      for (i = 0;
       ...
       CREATE(player_table[nr].unix_name, char, strlen(dummy.name) + 1);
       for (i = 0;
 	   (*(player_table[nr].unix_name + i) = LOWER(*(dummy.name + i))); i++);
...
/*test code*/
  /*  case 'C':
    else if(!strcmp(tag, "Cont"))
    GET_CONTACT(ch) = str_dup(line);*/
...
void check_start_rooms(void)
{
  ...
  if ((r_trans_space  = real_room(trans_space)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_trans_earth  = real_room(trans_earth)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_trans_vegeta  = real_room(trans_vegeta)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_trans_frigid  = real_room(trans_frigid)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_trans_namek  = real_room(trans_namek)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_trans_orainas  = real_room(trans_orainas)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_trans_konack  = real_room(trans_konack)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_konack_start_room  = real_room(konack_start_room)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_demon_start_room  = real_room(demon_start_room)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_majin_start_room  = real_room(majin_start_room)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_saiyan_start_room  = real_room(saiyan_start_room)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_angel_start_room  = real_room(angel_start_room)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_pk1_room  = real_room(pk1_room)) < 0) {
    log("SYSERR: pk room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_pk2_room  = real_room(pk2_room)) < 0) {
    log("SYSERR:  pk room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_pk3_room  = real_room(pk3_room)) < 0) {
    log("SYSERR:  pk room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_pk4_room  = real_room(pk4_room)) < 0) {
    log("SYSERR:  pk room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_icer_start_room  = real_room(icer_start_room)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_truffle_start_room  = real_room(truffle_start_room)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_namek_start_room  = real_room(namek_start_room)) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  if ((r_hell_room  = real_room(hell_room )) < 0) {
    log("SYSERR:  Mortal start room does not exist.  Change in config.c.");
    exit(1);
  }
  ...
  if ((r_frozen_start_room = real_room(frozen_start_room)) < 0) {
    ...
    if (!mini_mud)
      log("SYSERR:  Warning: Death start room does not exist.  Change in config.c.");
    r_death_start_room = r_mortal_start_room ;
  }
}
...
void parse_simple_mob(FILE *mob_f, int i, int nr)
{
  ...
  mob_proto[i].real_abils.rage = 10;
  mob_proto[i].real_abils.ubs = 50;
  mob_proto[i].real_abils.lbs = 50;
  ...
  mob_proto[i].player.eye = t[3];
  mob_proto[i].player.hairc = t[4];
  mob_proto[i].player.hairl = t[5];
  mob_proto[i].player.aura = t[6];
  mob_proto[i].player.clan = t[7];
  mob_proto[i].player.build = t[8];
  mob_proto[i].player.god = t[9];
...
void interpret_espec(const char *keyword, const char *value, int i, int nr)
{
  ...
  CASE("NumAttacks") {
    RANGE (2, 2);  /* limits setting the attacks from 0 to 10 */
...
void zone_update(void)
{
  ...
  /* jelson 10/22/92 */
  /*if (((++atimer * PULSE_ZONE) / PASSES_PER_SEC) >= 30 && !IS_NPC(ch)) {
     GET_ADD(ch) = 0;
     atimer = 0;
   }*/
  ...
  for (update_u = reset_q.head; update_u; update_u = update_u->next)
	...
	is_empty(update_u->zone_to_reset)) {
      ...
      sprintf(buf, "&10Auto zone reset: %s",
...
void store_to_char(struct char_file_u * st, struct char_data * ch)
{
  ...
  GET_SEX(ch) = st->sex;
  GET_EYE(ch) = st->eye;
  GET_HAIRC(ch) = st->hairc;
  GET_HAIRL(ch) = st->hairl;
  GET_AURA(ch) = st->aura;
  GET_CLAN(ch) = st->clan;
  GET_BUILD(ch) = st->build;
  GET_GOD(ch) = st->god;
...
void char_to_store(struct char_data * ch, struct char_file_u * st)
{
  ...
  st->sex = GET_SEX(ch);
  st->eye = GET_EYE(ch);
  st->hairc = GET_HAIRC(ch);
  st->hairl = GET_HAIRL(ch);
  st->aura = GET_AURA(ch);
  st->clan = GET_CLAN(ch);
  st->build = GET_BUILD(ch);
  st->god = GET_GOD(ch);
...
void init_char(struct char_data * ch)
{
  ...
  if (top_of_p_table == 0) {
    ...
    ch->points.max_hit = 5000000000;
    ch->points.max_mana = 5000000000;
    ch->points.max_move = 4;
  }
  set_title(ch, NULL);
  ...
  if (ch->player.sex == SEX_MALE) {
    ch->player.weight = number(0, 0);
    ch->player.height = number(0, 0);
  } else {
    ch->player.weight = number(0, 0);
    ch->player.height = number(0, 0);
  }
  ...
  ch->real_abils.rage = 100;
  ch->real_abils.ubs = 100;
  ch->real_abils.lbs = 100;
