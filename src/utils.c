...
void mudlog(const char *str, int type, long long level, int file)
{
  ...
  sprintf(buf, "&00&02[&10 %s &00&02]&00\r\n", str);
...
/* Called when stop following persons, or stopping MAJIN */
...
void stop_follower(struct char_data * ch)
{
  ...
  if (AFF_FLAGGED(ch, AFF_MAJIN)) {
    act("You come to your senses as the &09M&00 dissapears from your forehead!", FALSE, ch, 0, ch->master, TO_CHAR);
    if (affected_by_spell(ch, SPELL_MAJIN))
      affect_from_char(ch, SPELL_MAJIN);
  } else {
  ...
  ch->master = NULL;
  REMOVE_BIT(AFF_FLAGS(ch), AFF_MAJIN | AFF_GROUP);
}
