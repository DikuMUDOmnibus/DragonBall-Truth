...
int Crash_save(struct obj_data * obj, FILE * fp, sh_int containerNum);
...
struct obj_data *Obj_from_store(struct obj_file_elem object)
{
  ...
  if (real_object(object.item_number) > -1) {
    ...
    /***********************************************************************/
    /* The next 3 object.variables are stored in the object's structure    */
    /* because the data will be used later for putting objects into the    */
    /* containers they belong in and wearing items where they are supposed */
    /* to be worn.                                                         */
    /***********************************************************************/
    obj->containerNum        = object.containerNum;
    obj->where_it_goes       = object.where_it_goes;
...
int Obj_to_store(struct obj_data * obj, FILE * fl)
{
  ...
  /**************************************/
  /* Objects go in inventory by default */
  /**************************************/
  object.eq_Bitvector = 0;
  object.where_it_goes = -1; 
	
  /**************************************/
  /* Get Object's worn location         */
  /**************************************/
  /*if (obj->worn_by) {
    SET_BIT(object.eq_Bitvector, IS_WORN);
    object.where_it_goes = obj->worn_on;
  }*/

  /**************************************/
  /* Get Container's number             */
  /**************************************/
  if (GET_OBJ_TYPE(obj) == ITEM_CONTAINER) {
    object.containerNum = obj->containerNum;
  }

  /**************************************/
  /* Get Containee's Container number   */
  /**************************************/
  if (obj->in_obj) {
    SET_BIT(object.eq_Bitvector, IN_CONTAINER);
    object.where_it_goes = obj->in_obj->containerNum;
  }
...
*/
{
  ...
  struct obj_data *obj, *hold_obj[max_obj_save];
  sh_int hold_obj_bitvector[max_obj_save];
  int obj_ctr = 0, hold_ctr, num_container;
  float num_of_days;
  ...
  while (!feof(fl)) {
    ...
    if (!feof(fl)) {
      ...
      /*===============================================================*/
      /* Purpose:                                                      */
      /* I am storing all of the players objects into an array because */
      /* all objects that belong in a container will be loaded before  */
      /* the container is loaded.                                      */     
      /*****************************************************************/
      hold_obj[num_objs] = ch->carrying;
      hold_obj_bitvector[num_objs] = object.eq_Bitvector;

      /**************************************************************/
      /* Count the number of objects retrieved from the object file */
      /**************************************************************/
      num_objs++;
    }
  }
  ...
  /*****************************************************************/
  /* FIND CONTAINERS IN THE ARRAY                                  */
  /*===============================================================*/
  /* Purpose:                                                      */
  /* At this point everything from the player object file has been */
  /* stored into an array. Now we go through the array and check   */
  /* for containers.                                               */     
  /*****************************************************************/
  for (obj_ctr = 0; obj_ctr < num_objs; obj_ctr++) {
    if (GET_OBJ_TYPE(hold_obj[obj_ctr]) == ITEM_CONTAINER) {
      /**************************************************************/
      /* A container has been found, assign the array element to    */
      /* num_container.                                             */
      /**************************************************************/
      num_container = obj_ctr;
      /***************************************************************/
      /* Now take another pass through the array looking for objects */
      /* that are flagged as IN_CONTAINER and have a ->where_it_goes */
      /* that matches the ->containerNum of the above container.     */
      /* Notice that we only need to check array element 0 through   */
      /* num_container rather than 0 through num_objs because all of */
      /* a container's object are loaded before the container. So it */
      /* is not neccesary to loop through every item in the array.   */
      /***************************************************************/
      for (hold_ctr = 0; hold_ctr < num_container; hold_ctr++) { 
        if (IS_SET(hold_obj_bitvector[hold_ctr], IN_CONTAINER) && hold_obj[hold_ctr]->where_it_goes == hold_obj[obj_ctr]->containerNum) {
          obj = hold_obj[hold_ctr];
          obj_from_char(obj);
          obj->where_it_goes = 0;
          obj_to_obj(hold_obj[hold_ctr], hold_obj[obj_ctr]);
        }
      }
    }
  }
  ...
  /*******************************************************************/
  /* Equip worn objects in the array:                                */
  /*=================================================================*/
  /* We're going through the array again looking for objects         */
  /* flagged IS_WORN and wearing them on the body part indicated     */
  /* by obj->where_it_goes. I did not do this above because I wanted */
  /* to fill up wearable containers before wearing them.             */
  /*******************************************************************/
  /*for (obj_ctr = 0; obj_ctr < num_objs; obj_ctr++) {
    if (IS_SET(hold_obj_bitvector[obj_ctr], IS_WORN)) {
      obj = hold_obj[obj_ctr];
      obj_from_char(obj);
      equip_char(ch, obj, obj->where_it_goes);
    }
  }*/


  /* Little hoarding check. -gg 3/1/98 */
  sprintf(fname, "%s (level %Ld) has %d objects (max %d).",
...
int Crash_save(struct obj_data * obj, FILE * fp, sh_int containerNum)
{
  ...
  if (obj) {
    ...
    if (GET_OBJ_TYPE(obj) == ITEM_CONTAINER && obj->contains) {
      containerNum = number(1, 50000000);
      obj->containerNum = containerNum;
    }
    Crash_save(obj->contains, fp, 0);
    Crash_save(obj->next_content, fp, 0);
    
    result = Obj_to_store(obj, fp);

    for (tmp = obj->in_obj; tmp; tmp = tmp->in_obj)
      GET_OBJ_WEIGHT(tmp) == GET_OBJ_WEIGHT(tmp);
...
void Crash_crashsave(struct char_data * ch)
{
  ...
  for (j = 0; j < NUM_WEARS; j++)
    if (GET_EQ(ch, j)) {
      if (!Crash_save(GET_EQ(ch, j), fp, 0)) {
	fclose(fp);
	return;
      }
