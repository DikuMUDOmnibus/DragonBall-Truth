...
void random_mtrigger(char_data *ch)
{
  ...
  if (!SCRIPT_CHECK(ch, MTRIG_RANDOM) || AFF_FLAGGED(ch, AFF_MAJIN))
    return;
...
void bribe_mtrigger(char_data *ch, char_data *actor, int amount)
{
  ...
  if (!SCRIPT_CHECK(ch, MTRIG_BRIBE) || AFF_FLAGGED(ch, AFF_MAJIN))
    return;
...
void entry_memory_mtrigger(char_data *ch)
{
  ...
  if (!SCRIPT_CHECK(ch, MTRIG_MEMORY) || AFF_FLAGGED(ch, AFF_MAJIN))
    return;
  if (!SCRIPT_MEM(ch))
    return;
...
int entry_mtrigger(char_data *ch)
{
  ...
  if (!SCRIPT_CHECK(ch, MTRIG_ENTRY) || AFF_FLAGGED(ch, AFF_MAJIN))
    return 1;
...
int command_mtrigger(char_data *actor, char *cmd, char *argument)
{
  ...
  for (ch = world[IN_ROOM(actor)].people; ch; ch = ch_next) {
    ...
    if (SCRIPT_CHECK(ch, MTRIG_COMMAND) && !AFF_FLAGGED(ch, AFF_MAJIN) &&
        (actor!=ch)) {
      for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
...
void speech_mtrigger(char_data *actor, char *str)
{
  ...
  for (ch = world[IN_ROOM(actor)].people; ch; ch = ch_next)
  {
    ...
    if (SCRIPT_CHECK(ch, MTRIG_SPEECH) && AWAKE(ch) &&
        !AFF_FLAGGED(ch, AFF_MAJIN) && (actor!=ch))
      for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
		  ...
		  obj_data *target, char *arg)
{
  ...
  if (SCRIPT_CHECK(ch, MTRIG_ACT) && !AFF_FLAGGED(ch, AFF_MAJIN) &&
      (actor!=ch))
    for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next)  {
...
void hitprcnt_mtrigger(char_data *ch)
{
  ...
  if (!SCRIPT_CHECK(ch, MTRIG_HITPRCNT) || !FIGHTING(ch) ||
      AFF_FLAGGED(ch, AFF_MAJIN))
    return;
  perc1 = GET_HIT(ch) / 100;
  perc2 = GET_MAX_HIT(ch) / 100;
  for (t = TRIGGERS(SCRIPT(ch)); t; t = t->next) {
    if (TRIGGER_CHECK(t, MTRIG_HITPRCNT) && GET_MAX_HIT(ch) &&
	(((perc1 * 100) / perc2) <= GET_TRIG_NARG(t))) {
...
int receive_mtrigger(char_data *ch, char_data *actor, obj_data *obj)
{
  ...
  if (!SCRIPT_CHECK(ch, MTRIG_RECEIVE) || AFF_FLAGGED(ch, AFF_MAJIN))
    return 1;
...
int death_mtrigger(char_data *ch, char_data *actor)
{
  ...
  if (!SCRIPT_CHECK(ch, MTRIG_DEATH) || AFF_FLAGGED(ch, AFF_MAJIN))
    return 1;
