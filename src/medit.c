...
#define GET_NUM_ATTACKS(mob) ((mob)->mob_specials.num_attacks)
...
void medit_save_to_disk(int zone_num)
{
  ...
  if (!(mob_file = fopen(fname, "w"))) {
    mudlog("SYSERR: OLC: Cannot open mob file!", BRF, LVL_IMMORT, TRUE);
    return;
  }
  ...
  for (i = zone * 100; i <= top; i++) {
	...
	fprintf(mob_file, "BareHandAttack: %d\n", GET_ATTACK(mob));
      if (GET_NUM_ATTACKS(mob) != 0)
        fprintf(mob_file, "NumAttacks: %d\n", GET_NUM_ATTACKS(mob));
      if (GET_STR(mob) != 11)
...
"[H[J"
	  ...
	  "%s4%s) L-Desc:-\r\n%s%s"
	  "%s5%s) D-Desc:-\r\n%s%s"
     "%s6%s) Level:       [%s%9Ld%s],  %s7%s) Alignment:    [%s%4d%s]\r\n"
     ...
     "%sA%s) NumDamDice:  [%s%4d%s],  %sB%s) SizeDamDice:  [%s%4d%s]\r\n"
	  ...
	  "%sF%s) Armor Class: [%s%4d%s],  %sG%s) Exp:     [%s%9Ld%s],  %sH%s) Gold:  [%s%8d%s]\r\n",
...
/*-------------------------------------------------------------------*/
  case MEDIT_NUM_ATTACKS:
    GET_NUM_ATTACKS(OLC_MOB(d)) = MAX(0, MIN(10, atoi(arg)));
    break;
/*-------------------------------------------------------------------*/       
#if defined(OASIS_MPROG)
  case MEDIT_MPROG:
...
*/
  ...
  case MEDIT_HITROLL:
    GET_HITROLL(OLC_MOB(d)) = MAX(0, MIN(500, atoi(arg)));
    break;

  case MEDIT_DAMROLL:
    GET_DAMROLL(OLC_MOB(d)) = MAX(0, MIN(500000, atoi(arg)));
    break;

  case MEDIT_NDD:
    GET_NDD(OLC_MOB(d)) = MAX(0, MIN(300, atoi(arg)));
    break;

  case MEDIT_SDD:
    GET_SDD(OLC_MOB(d)) = MAX(0, MIN(10000, atoi(arg)));
    break;

  case MEDIT_NUM_HP_DICE:
    GET_HIT(OLC_MOB(d)) = MAX(0, MIN(1000, atoi(arg)));
    break;

  case MEDIT_SIZE_HP_DICE:
    GET_MANA(OLC_MOB(d)) = MAX(0, MIN(1000000000, atoi(arg)));
    break;

  case MEDIT_ADD_HP:
    GET_MOVE(OLC_MOB(d)) = MAX(0, MIN(1000000000, atoi(arg)));
    break;
  ...
  case MEDIT_LEVEL:
    GET_LEVEL(OLC_MOB(d)) = MAX(1, MIN(10000000, atoi(arg)));
    break;

  case MEDIT_KI:
    GET_MAX_MANA(OLC_MOB(d)) = MAX(1, MIN(10000000, atoi(arg)));
    break;
...
/*-------------------------------------------------------------------*/
  default:
     ...
     */
    cleanup_olc(d, CLEANUP_ALL);
    mudlog("SYSERR: OLC: medit_parse(): Reached default case!", BRF, LVL_IMMORT, TRUE);
    send_to_char("Oops...\r\n", d->character);
    break;
  }
/*-------------------------------------------------------------------*/
