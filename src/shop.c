...
int is_ok_char(struct char_data * keeper, struct char_data * ch, int shop_nr)
{
  ...
  if ((IS_Human(ch) && NOTRADE_Human(shop_nr)) ||
      (IS_Namek(ch) && NOTRADE_Namek(shop_nr)) ||
      (IS_icer(ch) && NOTRADE_icer(shop_nr)) ||
      (IS_MAJIN(ch) && NOTRADE_MAJIN(shop_nr)) ||
      (IS_ANDROID(ch) && NOTRADE_ANDROID(shop_nr)) ||
...
int trade_with(struct obj_data * item, int shop_nr)
{
  ...
  for (counter = 0; SHOP_BUYTYPE(shop_nr, counter) != NOTHING; counter++)
    if (SHOP_BUYTYPE(shop_nr, counter) == GET_OBJ_TYPE(item)) {
	  ...
	  ((GET_OBJ_TYPE(item) == ITEM_SCOUTER) ||
		       ...
		       struct char_data * keeper, int shop_nr)
{
  ...
  *buf = '\0';
  ...
  if (buynum > 20 && buynum <= 49) {
   sprintf(buf, "&14%s&10 has bought %d items at one time at %s.",
            GET_NAME(ch), buynum, world[ch->in_room].name);
  ...
  commafmt(price, sizeof(price), goldamt);
  sprintf(buf, "%s That'll be %s zenni, thanks.", GET_NAME(ch), price);
  do_tell(keeper, buf, cmd_tell, 0);
		            ...
		            struct char_data * keeper, int shop_nr, int msg)
{
  ...
  case OBJECT_NOTOK:
    ...
    break;
  case OBJECT_DEAD:
    sprintf(buf, "%s %s", GET_NAME(ch), MSG_NO_USED_SCOUTERSTAFF);
    break;
  default:
		        ...
		        struct char_data * keeper, int shop_nr)
{
  ...
  if (sold < sellnum) {
   ...
   commafmt(price, sizeof(price), goldamt);
   sprintf(buf, "%s I'll give you %s zenni, nice doing business with you.", GET_NAME(ch), price);
   do_tell(keeper, buf, cmd_tell, 0);
		         ...
		         struct char_data * keeper, int shop_nr)
{
  ...
  if (!(obj = get_selling_obj(ch, name, keeper, shop_nr, TRUE)))
    return;
  commafmt(price, sizeof(price), sell_price(ch, obj, shop_nr));
   sprintf(buf, "%s I'll give you %s zenni for that!", GET_NAME(ch),
	  price);
   do_tell(keeper, buf, cmd_tell, 0);
...
char *list_object(struct obj_data * obj, int cnt, int index, int shop_nr)
{
  ...
  char price[500];
  int valu = buy_price(obj, shop_nr);

  if (shop_producing(obj, shop_nr))
    strcpy(buf2, "&15Unlimited   ");
  else
    sprintf(buf2, "  %5d       ", cnt);
  sprintf(buf, " &14%2d&16)&15  %s", index, buf2);
  ...
  if ((GET_OBJ_TYPE(obj) == ITEM_SCOUTER) || (GET_OBJ_TYPE(obj) == ITEM_STAFF))
    if (GET_OBJ_VAL(obj, 2) < GET_OBJ_VAL(obj, 1))
      strcat(buf3, " (partially used)");
  commafmt(price, sizeof(price), valu);
  sprintf(buf2, "%s &11%6s&00\r\n", buf3, price);
  strcat(buf, CAP(buf2));
  return (buf);
}
		        ...
		        struct char_data * keeper, int shop_nr)
{
  ...
  one_argument(arg, name);
  strcpy(buf, "&00&01 ##   &15Available   &15Item                                               &14C&12o&00&04st&00\r\n");
  strcat(buf, "&00&06-------------------------------------------------------------------------&00\r\n");
  if (keeper->carrying)
  ...
  sprintf(buf2, "&16[&15Your Zenni&00&01: &11%s&16]&00\r\n", gold);
